...
JETSON_VERSION='jetson-api jetson-amqp-wrapper'
STATUS='302 Found'
   ...
   do
     ...
     local GET_STATUS=$(curl -s  -I http://web-lab/bortnik/$V/-/jobs/artifacts/devops/download?job=build-job | grep -oP "302 Found")
         ...
         if [[ $JETSON_LATEST_VERSION != $JETSON_CURRENT_VERSION && $STATUS == $GET_STATUS ]]; then
            ...
            logger -i -t SCAN_DEB -p local0.info  "$DATE Found a new $V  artifact"
            curl  -L --output /tmp/$V.zip http://web-lab/bortnik/$V/-/jobs/artifacts/devops/download?job=build-job
            cd $REPO_PATH
            unzip -o /tmp/$V.zip
            rm rf /tmp/$V.zip
            RESULT=$(find $REPO_PATH -maxdepth  1 -type f -iname "*.deb*" ! -size 0 | awk -F "/" '{print $NF}')
              for i in $RESULT
                 ...
                 CHECK=$(file ${REPO_PATH}${i} | cut -d ' ' -f 2,3,4 )
                     ...
                     logger -i -t SCAN_DEB -p local0.info  "in upload was found non a Debian type of package. Only Debian package are allow and unknown will be delete"
                     rm -rf ${i}
              fi
               logger -i -t SCAN_DEB -p local0.info "the $i was found in upload directory"
               logger -i -t SCAN_DEB -p local0.info "move $i to repository"
